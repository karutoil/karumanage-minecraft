...
resolve_java_25() {
  local java_bin=""
  if [ -x "/usr/lib/jvm/temurin-25-jre/bin/java" ]; then
    java_bin="/usr/lib/jvm/temurin-25-jre/bin/java"
  elif command -v update-alternatives &>/dev/null; then
    java_bin=$(update-alternatives --list java 2>/dev/null | grep -E "/(temurin|jdk|java)-?25/" | head -1 || true)
  fi

  if [ -z "$java_bin" ]; then
    echo "    ✗ Java 25 binary not found on PATH or in /usr/lib/jvm"
